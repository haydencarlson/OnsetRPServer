...
function SetPlayerOnline(player)
		...
		local steamid = PlayerData[player].steamid
	  mariadb_query(sql, query)
	  mariadb_async_query(sql, query, OnLoadedData, player, steamid)
end
AddEvent("OnPlayerSteamAuth", SetPlayerOnline)

function OnLoadedData(player, steamid)
	local entry = mariadb_get_assoc(1)
	update_query = mariadb_prepare(sql, "UPDATE accounts SET online = '?' WHERE steamid = '?';",
		"true",	
		PlayerData[player].steamid
		)
		mariadb_query(sql, update_query)

		print(PlayerData[player].steamid, "hello?")
end